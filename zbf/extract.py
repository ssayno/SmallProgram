...
class ThreadDownloadImageAsync(threading.Thread):
    def __init__(self, url_and_paths):
        super().__init__()
        self.url_and_paths = url_and_paths

    def run(self) -> None:
        asyncio.run(self.main())

    async def crawl_single_image(self, session, url, image_path):
        async with session.get(url, headers=SIMPLE_HEADER) as response:
    ...
    async def main(self):
        ...
        for image_url, image_path in self.url_and_paths:
            tasks.append(
                self.crawl_single_image(session, image_url, image_path)
            )
...
def get_single_product(id_, pid, offset=0, limit=10):
    ...
    target_dir = os.path.join(CURRENT_DIR, f'{pid}', f'{id_}')
